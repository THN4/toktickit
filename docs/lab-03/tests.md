...
| VIS-02–05 | Mocked visual evidence | AC-01–21 | Authentication feedback and password change at desktop/tablet/mobile; Queue responsive/no-results/failure; Ticket Detail responsive controls/comments/notes; User Management validation. API is mocked with `page.route()` and each image is generated by Playwright. | `e2e/lab-03/visual-evidence.spec.ts` | Pass (4 tests; 22 screenshots) |
