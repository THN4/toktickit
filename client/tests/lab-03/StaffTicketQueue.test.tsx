...
describe('Lab 3 Staff Queue UI', () => {
 ...
 it('renders documented controls and empty state', async () => { render(<StaffQueuePage />); await waitFor(() => expect(screen.getByText(/No Tickets are currently/)).toBeInTheDocument()); expect(screen.getByLabelText('IT priority')).toBeInTheDocument(); expect(screen.getByLabelText('Owner ID')).toBeInTheDocument(); expect(screen.getByLabelText('Page size')).toBeInTheDocument(); });
